--[[
	...
	║    • 	Macro Play | Anti Macro Nerf | Per Map Macro          ║
	║    •  Shop Summon | Limited Summon | Auto Collect Eggs      ║
    ╚═════════════════════════════════════════════════════════════╝
--]]
...
if PlayerGui:FindFirstChild("RenderBlocker") then PlayerGui:FindFirstChild("RenderBlocker"):Destroy(); end
local RenderBlockerGUI = Instance.new("ScreenGui", PlayerGui);
RenderBlockerGUI.Name = "RenderBlocker";
RenderBlockerGUI.IgnoreGuiInset = true;
RenderBlockerGUI.ResetOnSpawn = false;
RenderBlockerGUI.Enabled = false;
local RenderBlockerFrame = Instance.new("Frame", RenderBlockerGUI);
RenderBlockerFrame.Size = UDim2.fromScale(1, 1);
RenderBlockerFrame.Position = UDim2.new(0, 0, 0, 0);
...
local MapMacroManagerBox1 = Tabs.Game:AddRightTabbox();
local MapMacroManagerBox2 = Tabs.Game:AddRightTabbox();
local ClassicMapMacroBox = MapMacroManagerBox1:AddTab("Classic");
local EndlessMapMacroBox = MapMacroManagerBox1:AddTab("Endless");
local SurvivalMapMacroBox = MapMacroManagerBox2:AddTab("Survival");
local OtherMapMacroBox = MapMacroManagerBox2:AddTab("Others");
...
local function RefreshMacroList()
    ...
    for _, file in pairs(UTILITY_LIBRARY:GetFiles(HUBPATH.."/"..SAVEPATH.."/macro")) do
        ...
        table.insert(MacroLists, (fileSplit[#fileSplit]):split(".")[1]);
    end
end
RefreshMacroList();
...
local SurvivalMapsMacro = {};
for _, map in pairs(DATA_LIBRARY["MAP_LIST"]) do
    if table.find(DATA_LIBRARY["MAPS"][map]["Allowed"], "Classic") then
    ...
    if table.find(DATA_LIBRARY["MAPS"][map]["Allowed"], "Endless") then
    ...
    if table.find(DATA_LIBRARY["MAPS"][map]["Allowed"], "Survival") then
        SurvivalMapsMacro[map] = SurvivalMapMacroBox:AddDropdown(DATA_LIBRARY["MAPS"][map]["ID"].."_survival", {
            Text = map,
            Values = MacroLists,
            Default = nil,
            Searchable = true,
...
MacroManagerTab:AddCheckbox("AutoAdjustMacroSpeed", {
    Text = "Auto Adjust Macro Speed",
    Default = false,
    Tooltip = "Automatically adjust macro speed according to the game's conditions.",
    Risky = true,
});
MacroManagerTab:AddButton({
    ...
    Func = function()
        ...
        for _, map in pairs(DATA_LIBRARY["MAP_LIST"]) do
            ...
            SurvivalMapsMacro[map]:SetValues(MacroLists);
        end
        task.wait(0.5);
        Library:Notify({
            Title = "NeoPulse";
...
for i = 1, 5 do
    MacroStatusLines[i] = MacroStatusBox:AddLabel("", false);
...
local x1GnomeSummon = GnomeSummonBox:AddButton({
    Text = "x1 | 100", Func = function() REMOTEFUNCTIONS_FOLDER["BuyUnitBox"]:InvokeServer(DATA_LIBRARY["SUMMON"]["SEEDS"][Options["GnomeUnitBox"].Value]["ID"], 1); end
});
local x10GnomeSummon = x1GnomeSummon:AddButton({
    Text = "x10 | 900", Func = function() REMOTEFUNCTIONS_FOLDER["BuyUnitBox"]:InvokeServer(DATA_LIBRARY["SUMMON"]["SEEDS"][Options["GnomeUnitBox"].Value]["ID"], 10); end
});

-- ── Event Summon ───────────────────────────────────────────────
EventSummonBox:AddLabel("Bunny Summon");
EventSummonBox:AddButton({
	Text = "x1 | 100", Func = function() REMOTEFUNCTIONS_FOLDER["BuyUnitBox"]:InvokeServer("ub_easter", 1); end
}):AddButton({
	Text = "x10 | 900", Func = function() REMOTEFUNCTIONS_FOLDER["BuyUnitBox"]:InvokeServer("ub_easter", 10); end
});
EventSummonBox:AddDivider();
EventSummonBox:AddLabel("Galaxy Summon");
EventSummonBox:AddButton({
	Text = "x1 | 25", Func = function() REMOTEFUNCTIONS_FOLDER["BuyUnitBox"]:InvokeServer("ub_space", 1); end
}):AddButton({
	Text = "x10 | 225", Func = function() REMOTEFUNCTIONS_FOLDER["BuyUnitBox"]:InvokeServer("ub_space", 10); end
});
...
EasterBox:AddCheckbox("EggTomatoEgg", {
    Text = "Auto Tomato Egg",
    Default = false,
    Callback = function()
        if Workspace:GetAttribute("IsLobby") == false then return; end
        if not Toggles["EggTomatoEgg"].Value then return; end
        for _ = 1, 100 do
            REMOTEEVENTS_FOLDER["IndexClick"]:FireServer("unit_tomato_plant");
            task.wait(1);
        end
    end
});
...
local RenderGroup = Tabs["UI Settings"]:AddRightGroupbox("Render Settings");
...
-- ── Render Settings ────────────────────────────────────────────
local RenderBlockerToggle = RenderGroup:AddToggle("EnableRenderBlocker", {
    Text = "Enable Render Blocker",
    Default = false,
    Tooltip = "Blocks the game's render by putting a frame in front of the screen. Useful for hiding render-heavy scenes like the lobby obby.",
    Callback = function(value) RenderBlockerGUI.Enabled = value; end
});
RenderBlockerToggle:AddColorPicker("RenderBlockerColor", {
    Text = "Render Blocker Color",
    Default = Color3.new(1, 1, 1),
});
Options["RenderBlockerColor"]:OnChanged(function()
    RenderBlockerFrame.BackgroundColor3 = Options["RenderBlockerColor"].Value;
end)
RenderBlockerFrame.BackgroundColor3 = Options["RenderBlockerColor"].Value or Color3.new(1, 1, 1);
...
if Workspace:GetAttribute("IsLobby") == false then
    ...
    local function TimeConvert(inputTime, inputSpeed, targetSpeed)
        local rateInput = 4 * inputSpeed / (13.5 + inputSpeed);
        local H = inputTime * rateInput;
        local rateTarget = 4 * targetSpeed / (13.5 + targetSpeed);
        return math.ceil(H / rateTarget);
    end
    ...
    local function ValidateCondition(Condition)
        if Condition["Type"] == "Time" then
            ...
            local ConditionTime = Condition["Data"];
            if Toggles["AutoAdjustMacroSpeed"].Value then
                ConditionTime = TimeConvert(ConditionTime, Condition["Speed"], Workspace:GetAttribute("TickSpeed"));
            end
            if TimeDifference >= ConditionTime then return true; end
            return false;
        end
        ...
        while task.wait() do
            ...
            task.spawn(function()
                ...
                task.spawn(function()
                    ...
                    if MacroData[Counter]["Condition"]["Type"] == "Time" then
                        if Toggles["AutoAdjustMacroSpeed"].Value then
                            local ConvertedTime = TimeConvert(MacroData[Counter]["Condition"]["Data"], MacroData[Counter]["Condition"]["Speed"], Workspace:GetAttribute("TickSpeed"));
                            MacroStatusLines[1]:SetText("Condition: "..MacroData[Counter]["Condition"]["Type"].." | "..string.format("%02i:%02i", math.floor(ConvertedTime / 60), ConvertedTime % 60));
                        else
                            local Time = MacroData[Counter]["Condition"]["Data"];
                            MacroStatusLines[1]:SetText("Condition: "..MacroData[Counter]["Condition"]["Type"].." | "..string.format("%02i:%02i", math.floor(Time / 60), Time % 60));
                        end
                    elseif MacroData[Counter]["Condition"]["Type"] == "Cash" then
                        MacroStatusLines[1]:SetText("Condition: "..MacroData[Counter]["Condition"]["Type"].." | $"..MacroData[Counter]["Condition"]["Data"]);
                    end
                end)
                if MacroData[Counter]["Task"] == "PlaceUnit" then
                    MacroStatusLines[2]:SetText("Task: Place Unit");
                    MacroStatusLines[3]:SetText("Unit: "..MacroData[Counter]["Unit"]);
                ...
                elseif MacroData[Counter]["Task"] == "UpgradeUnit" then
                    MacroStatusLines[2]:SetText("Task: Upgrade Unit");
                    MacroStatusLines[3]:SetText("Times: "..MacroData[Counter]["Upgrades"]);
                    MacroStatusLines[4]:SetText("ID: "..MacroData[Counter]["ID"]);
                    for i = 1, 4 do MacroStatusLines[i]:SetVisible(true); end
                    for i = 5, 5 do MacroStatusLines[i]:SetVisible(false); end
                elseif MacroData[Counter]["Task"] == "SellUnit" then
                    MacroStatusLines[2]:SetText("Task: Sell Unit");
            ...
            elseif MacroData[Counter]["Task"] == "AutoUpgrade" then

            elseif MacroData[Counter]["Task"] == "TargetPriority" then
    ...
    antiNerfThread = UTILITY_LIBRARY:Thread("AntiNerfThread", function()
        while task.wait(30) do
            if Library.Unloaded then break; end
            if not Toggles["EnableRandomWalk"].Value and not Toggles["EnableRandomJump"].Value then continue; end

            if Toggles["EnableRandomWalk"].Value then
                if math.random(1, 4) <= 2 then
                    local randomDirection = LocalPlayer.Character["HumanoidRootPart"].Position + Vector3.new(math.random(-10, 10), 0, math.random(-10, 10));
                    LocalPlayer.Character["Humanoid"]:MoveTo(randomDirection);
                end
            end
            if Toggles["EnableRandomJump"].Value then
                if math.random(1, 4) <= 2 then
...
-- ═══════════════════════════════════════════════════════════════
-- FEATURE: GNOME SHOP | LIMITED UNITS | TRADERS
-- ═══════════════════════════════════════════════════════════════
...
autoCollectEggsConnection = Workspace.ChildAdded:Connect(function(part) if Toggles["AutoCollectEggs"].Value then CollectSpawnedEggs(part); end end)
if Toggles["AutoCollectEggs"].Value then for _, part in pairs(Workspace:GetChildren()) do CollectSpawnedEggs(part); end end
...
Library:OnUnload(function()
	...
	if autoCollectEggsConnection then autoCollectEggsConnection:Disconnect(); autoCollectEggsConnection = nil; end

    -- Disconnect Entity Handler
    if entityHandler then entityHandler:Disconnect(); entityHandler = nil; end

    -- Stop All Threads
    if macroThread then macroThread:Stop();  macroThread = nil;  end
    if gameThread then gameThread:Stop(); gameThread = nil; end
    if antiNerfThread then antiNerfThread:Stop(); antiNerfThread = nil; end

    -- Destroy Render Blocker
    PlayerGui:FindFirstChild("RenderBlocker"):Destroy();
